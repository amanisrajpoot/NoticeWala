"""
Real UPSC Crawler - Actually scrapes UPSC website for live exam notifications
"""
...
class RealUPSCrawler(BaseCrawler):
    """Real crawler for UPSC exam notifications"""
    ...
    def __init__(self):
        ...
        # Real UPSC URLs to scrape
        self.scrape_urls = [
            "https://upsc.gov.in/notification",
            "https://upsc.gov.in/current-notifications",
            "https://upsc.gov.in/previous-notifications"
        ]
    ...
    def extract_announcements(self, content: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            # UPSC typically uses specific patterns for notifications
            notification_links = soup.find_all('a', href=re.compile(r'/notification|/exam|/recruitment'))
            ...
            for link in notification_links:
                try:
                    ...
                    if not any(keyword in title.lower() for keyword in ['notification', 'exam', 'recruitment', 'examination']):
                        continue
                    ...
                    if len(cells) >= 2:
                        try:
                            ...
                            if link:
                                ...
                                if len(title) > 10 and any(keyword in title.lower() for keyword in ['notification', 'exam', 'recruitment']):
                                    announcement_data = self._scrape_notification_details(full_url, title)
                                    if announcement_data:
                                        announcements.append(announcement_data)
                        except Exception as e:
    ...
    def crawl_real_notifications(self) -> List[Dict[str, Any]]:
        """Crawl real UPSC notifications"""
        all_announcements = []
        
        try:
            logger.info("Starting real UPSC notification crawl")
            ...
            logger.error(f"Error in real UPSC crawl: {e}")
            return []
    ...
    def run_crawl(self) -> Dict[str, Any]:
        ...
        try:
            logger.info("Starting real UPSC crawl")
            ...
            logger.info("Real UPSC crawl completed", **result)
            return result
            
        except Exception as e:
            logger.error("Real UPSC crawl failed", error=str(e))
            return {
                "success": False,
                "source": self.name,
                "error": str(e),
    ...
    def save_announcements(self, announcements: List[Dict[str, Any]]) -> int:
        ...
        try:
            ...
            db.commit()
            logger.info(f"Saved {saved_count} new announcements from real UPSC crawl")
